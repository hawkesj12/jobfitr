...
# ── a named place beats a stray remote_only ──────────────────────────────────
# Same shape as the exclusion bug: the assistant writes two fields that disagree and the
# contract resolved silently in favour of the one that DELETES. `remote_only` is a hard filter
# in server._eligible, so `location="Louisville, KY"` + `remote_only=true` removed every job in
# Louisville. chat.py's prompt says "if they say remote, set remote_only=true" with no rule
# against combining it with a city, and one real search already carried
# `location='anywhere' + remote_only=true`, so the shape is reachable and observed.


def test_a_named_city_survives_a_stray_remote_only_flag():
    """THE BUG. Honouring the flag empties the board and offers no way back; honouring the city
    shows some extra onsite jobs the user can remove with one chip. The asymmetry decides it."""
    notes = []
    cfg = config_from_dict(
        {"titles": ["Warehouse Supervisor"], "location": "Louisville, KY", "remote_only": True},
        notes,
    )
    assert cfg.location == "Louisville, KY"
    assert cfg.remote_only is False, "remote-only would delete every job in the named city"
    assert notes and "Louisville, KY" in notes[0]


def test_remote_plus_the_flag_is_agreement_not_a_contradiction():
    cfg = config_from_dict({"titles": ["X"], "location": "remote", "remote_only": True}, [])
    assert cfg.remote_only is True


def test_anywhere_plus_the_flag_is_coherent_and_the_flag_stands():
    """"Anywhere" is a statement about GEOGRAPHY; remote-only is one about ARRANGEMENT. Wanting
    remote work from anywhere is coherent, so this must NOT be treated as a contradiction — the
    guard has to be narrow or it starts overriding things users meant."""
    notes = []
    cfg = config_from_dict(
        {"titles": ["X"], "location": "anywhere", "remote_only": True}, notes
    )
    assert cfg.remote_only is True
    assert notes == []


def test_a_flag_with_no_location_still_wins():
    cfg = config_from_dict({"titles": ["X"], "remote_only": True}, [])
    assert cfg.remote_only is True


def test_remote_only_FALSE_alongside_a_city_is_untouched():
    """The guard only fires on the DELETING direction."""
    cfg = config_from_dict(
        {"titles": ["X"], "location": "Louisville, KY", "remote_only": False}, []
    )
    assert cfg.remote_only is False


def test_both_overrides_report_through_the_SAME_notes_channel():
    """Review's advice: reuse this channel, do not add a second notes-shaped one."""
    notes = []
    config_from_dict(
        {
            "titles": ["Warehouse Supervisor"],
            "exclude": ["warehouse supervisor"],
            "location": "Louisville, KY",
            "remote_only": True,
        },
        notes,
    )
    assert len(notes) == 2, f"expected both overrides reported, got {notes}"
